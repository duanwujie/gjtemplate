...
{	
	...
	if(count_of_basket_buy_order>0)PbPips=ND((Bid-BEbasket)/Pip,1);
	if(count_of_basket_sell_order>0)PbPips=ND((BEbasket-Ask)/Pip,1);
	...
	{	if(CountOfBasketOrder==NO_BASKET_ORDERS)
		...
		if(CountOfBasketOrder>0)
		{	if((PbPips+PhPips*total_lots_of_hedge_order/total_lots_of_basket_order)<0&&SLb>0)SLb=0;
			if(SLb>0&&(PbPips+PhPips*total_lots_of_hedge_order/total_lots_of_basket_order)<=SLb)
			...
			{	double TPbMP=ND(PbTarget*ProfitSet,Digits);
				if((PbPips+PhPips*total_lots_of_hedge_order/total_lots_of_basket_order)>PbTarget*ProfitSet)
